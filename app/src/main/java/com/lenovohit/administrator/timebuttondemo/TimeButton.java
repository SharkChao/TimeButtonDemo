...
public class TimeButton extends android.support.v7.widget.AppCompatButton implements View.OnClickListener{
    ...
    public TimeButton(Context context, @Nullable AttributeSet attrs) {
        ...
        timedBg = array.getResourceId(R.styleable.TimeButton_tbTimedBackground, Color.RED);
        timeingBg = array.getResourceId(R.styleable.TimeButton_tbTimeingBackground, Color.DKGRAY);
    ...
    public void initTime(){
        Date data = UserData.getData();
        if (data ==null){
            timeCount=new TimeCount(Long.parseLong(countTime)*1000,Long.parseLong(countTime)*1000);
        }else {
